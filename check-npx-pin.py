...
def main() -> int:
    ...
    if missing:
        ...
        print(
            ...
            "      файл переехал, проверка стала мерить меньше и продолжила\n"
            "      рапортовать успех.\n"
            "FIX:  1) файл переехал — поправьте путь в SURFACES внутри скрипта;\n"
            "      2) удалён намеренно — уберите строку из SURFACES;\n"
            "      3) запускать из рабочего корня либо с --root <путь>."
        )
        return 1
    ...
    print(
        "\nWHAT: в тексте `npx layero` без `@latest`.\n"
        "WHY:  `npx` НЕ ходит в реестр, если пакет уже установлен локально или\n"
        "      глобально — он запускает то, что стоит. Это пин на годы, а не\n"
        "      разовая мелочь: 02.08 нашлись рабочий ноутбук на 0.8.11 при\n"
        "      опубликованном 0.8.20 и пользователь на 0.1.x, у которого деплой\n"
        "      не работал в принципе.\n"
        "FIX:  1) пишите `npx layero@latest` — везде, включая примеры и подсказки;\n"
        "      2) правя список, правьте пример рядом: копируют как раз пример;\n"
        f"      3) строгие поверхности задаёт STRICT в этом скрипте: {STRICT}."
    )
    return 1
