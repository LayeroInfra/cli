...
#
# 🚨 ЧИСЛО ОДИН РАЗ ВЫРОСЛО: 7 → 8, и это записано здесь, а не замолчано.
# Причина — `archive_snapshot.py`, вход этапа A: он единственный считает
# вердикт на ПОЛНОМ снимке и потому кандидат остаться единственным вообще.
# Условие сделки: его потолок держится на 1, а остальные обязаны падать до
# нуля по мере перехода потребителей на вмороженный план. Если через месяц
# сумма не ниже восьми — сделка не выполнена, и это видно прямо отсюда.
CEILINGS: dict[str, int] = {
    "backend/app/services/archive_snapshot.py": 1,    # этап A: полный снимок
