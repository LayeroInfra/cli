...
# 2. Тип приводится в соответствие с репозиторием — в обе стороны.
#
# 🚨 ЗДЕСЬ СТОЯЛА ОБРАТНАЯ ПРОВЕРКА: «уточнение обязано отказывать при
# source != 'default'». Владелец платформы снял её 26.08.2026, и посылка была
# неверной: человек эту ось не выбирает и о её существовании не знает — в
# мастере создания вопроса «SPA или SSR» нет вовсе. `source='user'` появлялся
# ПОСЛЕ отказа сборки, когда человек шёл в настройки переключать тип, разбираясь
# с ошибкой. Защита срабатывала на значении, которое сама же и породила.
#
# Теперь гейт стережёт ПРОТИВОПОЛОЖНОЕ: вето не должно вернуться, а ярус
# источника — остаться (он отвечает «кто последний трогал», это нужно отчётам).
check(
    "project_type_source = 'default'" not in q,
    "вето «выбор владельца» не вернулось",
    "уточнение типа снова отказывает при source != 'default' — ось опять "
    "блокирует сама себя: она становится 'user' от ручного переключения, "
    "которым человек чинит наш же отказ",
)
check(
    'source="detected"' in q,
    "автоматическое уточнение помечает источник",
    "без пометки источника отчёты перестанут отличать наше переключение от "
    "выбора человека",
)
