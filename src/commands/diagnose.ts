...
async function resolveDeployId(
  ...
  // Берём последний неуспешный, если он есть: спрашивают «почему упало»,
  // а не «как дела у позапрошлой удачной сборки». Если всё хорошо — самый
  // свежий.
  const failed = deploys.find((d) => d.status !== "ready" && d.status !== "queued" && d.status !== "building");
  const target = failed ?? deploys[0]!;
