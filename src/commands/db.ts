...
export async function dbConnectCmd(ref: string, opts: DbOptions): Promise<void> {
  ...
  // 🚨 СЛЕДСТВИЙ У СВЯЗИ ДВА, и второе стоило отдельного разбора 09.09.2026.
  // Кроме строки подключения, связь проект↔база — ЕДИНСТВЕННЫЙ источник списка
  // разрешённых доменов Data API: представление `data_api_origins` собирает их
  // из `userdb_roles`. Пока здесь говорилось только про переменные, человек,
  // у которого сайт отбивался по CORS, не имел ни одной причины связать это со
  // своей ненажатой командой — а отказ шлюза именно её и советует.
  console.log(
    `${chalk.green("✓")} проект «${project.slug}» подключён к базе «${db.name}»\n` +
      ...
      chalk.dim(`  домены проекта разрешены для Data API базы «${db.name}»`),
  );
}
...
export async function dbDisconnectCmd(ref: string, opts: DbOptions): Promise<void> {
  ...
  // Обратная сторона той же связи: домены проекта перестают быть разрешёнными.
  // Молчать об этом — значит отдать человеку сайт, который перестанет отвечать
  // по причине, названной в другой команде.
  console.log(
    `${chalk.green("✓")} проект «${project.slug}» отвязан от базы «${db.name}»\n` +
      ...
      chalk.dim("  домены проекта больше не разрешены для Data API этой базы"),
  );
}
