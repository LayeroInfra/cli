...
export interface paths {
    ...
    "/organizations/{slug}/databases/plans": {
         ...
         *
         *     Цена — НАША: живой прайс поставщика умножается на наценку
         *     (`timeweb_dbaas.our_price`). Признак `prices_are_provisional` теперь
         *     означает буквально одно: наценку никто не подтвердил явно, и цена —
         *     реконструкция прежней таблицы по измеренному отношению. Подтверждается
         *     ключом `userdb_dedicated_markup` в `platform_config`.
         *
         *     ⚠️ Прежде этот признак стоял в `True` БЕЗУСЛОВНО — то есть не отвечал ни
         *     на какой вопрос и потому никем не читался; панель дисклеймер убрала.
         *
