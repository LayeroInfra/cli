...
TypeScript, публикуется в npm как `layero` (на 19.09.2026 — `0.11.7`). Второй вход в платформу помимо привязки репозитория:
...
| `src/detect.ts` | локальное определение типа приложения поверх `layero-detection` + подсказки по форме папки (монорепо, фронт+бэк, свой скрипт, сервер) |
...
7. **Claimable — сайт без аккаунта** (`deploy --claim`): временный проект на
   час (с 19.09.2026, было 72 часа), только статика и SPA — серверное
   приложение отклоняется до выгрузки (`claim_static_only`); адрес случайный,
   поисковикам закрыт. Токен, код заявки и ссылка — в `~/.layero/config.json`
   (0600); в `.layero/project.json` только привязка: файл уходит в git, а код
   забора в публичном репозитории отдаёт сайт первому встречному (с 0.11.8,
   T-20260921; старое поле `claim` читается и при следующей выкатке уезжает
   в конфиг). Принять заявку может только человек в панели. Включается
   ТОЛЬКО явным `--claim` (с 0.11.8): раньше режим заводился сам у агента
   без входа с `--yes`, то есть публикация шла без решения человека. Песочница — только для
...
8. **Детект CLI — подсказка, а не решение.** В проект уходит только
   названное человеком: `--type`, `--root`, поля `.layero/project.json`,
   которые он написал сам; незаданное уходит `null` — «решит сборщик по
   архиву». До 0.11 первая выкатка сохраняла в проект догадку CLI, и сборщик
   исполнял её на каждой сборке: `static`, угаданный для монорепо или своего
   скрипта сборки, глушил сборку навсегда (T-20260918-7). `confident: true`
   только там, где папку узнали; иначе событие несёт `hint` и `next_action`.
   `deploy --dry-run` показывает план без выгрузки и без входа.
9. **`ready` — когда адрес отвечает сайтом.** CLI опрашивает `url`, пока
   ответ несёт `X-Layero-Screen` (экран платформы), до 90 с; `edge_ready` —
   итог этого опроса, а не проба платформы (T-20260918-8).
