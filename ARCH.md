...
TypeScript, публикуется в npm как `layero` (на 16.08.2026 — `0.8.24`,
...
| `src/commands/` | 18 команд: `deploy`, `login`, `projects`, `domains`, `env`, `data`, … |
...
2. **Внутри агента вывод переключается на JSON-lines** автоматически
   (`CURSOR_AGENT`, `CLAUDECODE` или не-TTY). Событие `ready` несёт живой
   адрес — его и показывать, **не собирая имя хоста по шаблону**: адреса
   живут в зоне `layero.app`, а непереехавшие организации — ещё на
   `layero.ru`.
3. **`deploy` публикует на апекс.** Прямые загрузки автопромоутятся; `--prod`
   ...
   CLI нет: `--branch` принимается и **молча игнорируется**, архивы всегда
   ложатся в зарезервированное окружение `cli`. Хотите превью — нужен
   подключённый репозиторий и push в ветку.
...
Публикацию ведёт `publish-cli.yml` при изменениях в `cli/**`. Версия
поднимается в `package.json`; после публикации **проверять живым** —
